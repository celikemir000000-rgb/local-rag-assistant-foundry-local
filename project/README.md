...
- evaluation_results.csv - Generated by evaluate_rag.py when an evaluation is run.
- evaluation_results_formatted.xlsx - Included formatted snapshot of the prepared evaluation results.
...
The initial dependency and model downloads require an internet connection.
After they are installed, normal document ingestion and question answering run
locally without a cloud inference API.
...
This creates the local `documents/rag_database.db` file. The database is not
...
New PDF files and generated SQLite databases are ignored by Git by default so
private document collections are not published accidentally. The six included
PDFs are intentionally public educational samples.
